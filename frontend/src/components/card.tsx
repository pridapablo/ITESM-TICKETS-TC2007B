    ...
    return (
        <div className="flex text-center">
            <div className="p-4 sm:w-3/3 lg:w-5/5 w-full hover:scale-105 duration-500">
                <div className={`flex items-center justify-between p-4 rounded-lg ${theme.palette.mode == 'dark'? 'bg-black': 'bg-white'} shadow-neutral-600 shadow-md`}>
                        <div className={`border ${theme.palette.mode == 'dark'? 'border-black': 'border-white'} w-[65%]`}>
                            <h2 className={`${theme.palette.mode == 'dark'? 'text-white': 'text-black'} text-lg font-bold`}>{ticketID}</h2>
                            <h3 className={`mt-2 text-xl font-bold ${theme.palette.mode == 'dark'? 'text-white': 'text-black'}`}>{props.classification}</h3>
                            <h3 className={`text-base font-bold ${theme.palette.mode == 'dark'? 'text-white': 'text-black'}`}>{props.subclassification}</h3>

                            {/* <p className=" text-base font-semibold text-gray-400">{props.classification}</p> */}
                            <p className=" pt-5 text-xs font-semibold text-gray-400 text-left">{props.description}</p>
                            <button onClick={handleCardClick} className="text-``sm mt-6 px-4 py-2 bg-green-600 text-white rounded-lg  tracking-wider hover:bg-green-800 outline-none">{role=="user"? "Editar": "Resolver" }</button>
                        </div>
                        <div className={`bg-gradient-to-tr ${priorColor}  w-32 h-32  rounded-full shadow-2xl border-white  border-dashed border-2  flex justify-center items-center `}>
                            <div>
