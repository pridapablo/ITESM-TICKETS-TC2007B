...
const calculateAvgResolutionTimeAllTime = async () => {
    try {
        const aggregationPipeline = [
            {
                $lookup: {
...
const calculateTicketCountsAllTime = async () => {
    try {
        const aggregationPipeline = [
            {
                $lookup: {
...
const calculateMostReportedCategoriesAllTime = async () => {
    try {
        // Sample MongoDB aggregation pipeline (customize as per your data model)
        const aggregationPipeline = [
            {
...
const calculateTicketStatsAllTime = async () => {
    try {
        // Current date and start of the week (7 days ago)
        const now = new Date();
        const startOfWeek = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
...
//@ts-ignore
const calculateInventoryByClassificationAllTime = async () => {
    try {
        const aggregationPipeline = [
            {
                $lookup: {
...
const fetchTicketsWithResolutionTimeAllTime = async () => {
    try {
        const aggregationPipeline = [
            {
                $lookup: {
...
export const getReport = async (_req, res) => {
    try {
        ...
        const ticketCountsAllTime = await calculateTicketCountsAllTime();
        const avgResolutionTimeAllTime = await calculateAvgResolutionTimeAllTime();
        const allResoultionTimeAllTime = await fetchTicketsWithResolutionTimeAllTime();
        const mostReportedCategoriesAllTime = await calculateMostReportedCategoriesAllTime();
        const ticketStatsAllTime = await calculateTicketStatsAllTime();
        const inventoryByClassificationAllTime = await calculateInventoryByClassificationAllTime();

        
        res.json(
            {dataWeek: { ticketCounts, avgResolutionTime, allResoultionTime, mostReportedCategories, ticketStats, inventoryByClassification},
            allData:{ticketCountsAllTime, avgResolutionTimeAllTime, allResoultionTimeAllTime, mostReportedCategoriesAllTime, ticketStatsAllTime, inventoryByClassificationAllTime}});
    } catch (error) {
        console.error(error);
